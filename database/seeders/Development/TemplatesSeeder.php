...
class TemplatesSeeder extends Seeder
{
    public function run(): void
    {
        ...
        // ── AkoFlow + GKE template ────────────────────────────────────────────
        $akoflowGkeTemplate = ExperimentTemplate::firstOrCreate(
            ['slug' => 'akoflow-gke'],
            [
                'name' => 'AkoFlow + GKE',
        ...
        // Update definition if version already exists (idempotent re-seed)
        $existingVersion = ExperimentTemplateVersion::where('template_id', $akoflowGkeTemplate->id)
            ->where('version', '1.0.0')
            ->first();

        $akoflowDefinition = self::akoflowGkeDefinition();

        if ($existingVersion) {
            $existingVersion->update(['definition_json' => $akoflowDefinition, 'is_active' => true]);
        } else {
            ExperimentTemplateVersion::create([
                'template_id' => $akoflowGkeTemplate->id,
                'version' => '1.0.0',
                'is_active' => true,
                'definition_json' => $akoflowDefinition,
            ]);
        }
    }

    private static function akoflowGkeDefinition(): array
    {
        return [
